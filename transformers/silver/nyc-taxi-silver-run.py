...
def transform_silver(df_bronze, year, month):
    df = df_bronze.filter((col("year") == year) & (col("month") == month))
...
def main():
    ...
    df_bronze = spark.read.table("nyctaxi_bronze.nyc_taxi_yellowbronze")
    df_silver = transform_silver(df_bronze, year, month)

    df_silver.writeTo("glue_catalog.nyctaxi_silver.nyc_taxi_yellow_silver_tratada").overwritePartitions()
    job.commit()
